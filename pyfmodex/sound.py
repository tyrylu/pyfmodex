...
class ConeSettings(object):
    def __init__(self, sptr):
        ...
        ckresult(
            _dll.FMOD_Sound_Get3DConeSettings(
    ...
    def _commit(self):
        ckresult(
            _dll.FMOD_Sound_Set3DConeSettings(
...
class Sound(FmodObject):
    ...
    def format(self):
        ...
        return so(
            type=SOUND_TYPE(type.value),
            format=SOUND_FORMAT(format.value),
    ...
    def set_loop_points(self, start, start_unit, end, end_unit):
        self._call_fmod(
            "FMOD_Sound_SetLoopPoints", start, int(start_unit), end, int(end_unit)
        )
