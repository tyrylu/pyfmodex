...
class DSP(FmodObject):
    def add_input(self, input, connection_type):
        ...
        connptr = c_void_p()
        self._call_fmod(
            "FMOD_DSP_AddInput", input._ptr, byref(connptr), connection_type.value
        )
    ...
    def channel_format(self, format):
        self._call_fmod(
            ...
            int(format.channel_mask),
            format.num_channels,
            format.source_speaker_mode.value,
        )
    ...
    def output_channel_format(self):
        ...
        )  # The parameter should be an SPEAKERMODE, but the echo dsp returned basically random value there.
